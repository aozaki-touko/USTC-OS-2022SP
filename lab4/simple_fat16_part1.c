    ...
    for (uint i = 1; Dir.DIR_Name[0] != 0x00; i++)
    {
      ...
      if (i % 16 == 0)
      {
        ...
        else // 当前簇已经读完，需要读取下一个簇的内容
        {
          ...
          /*** BEGIN ***/
          ClusterN = FatClusEntryVal; //update
               ...
               struct fuse_file_info *fi)
{
  ...
  WORD begin_cluster_num = begin_bytes / (BYTES_PER_SECTOR * fat16_ins->Bpb.BPB_SecPerClus);
  WORD end_cluster_num = end_bytes / (BYTES_PER_SECTOR * fat16_ins->Bpb.BPB_SecPerClus);
  WORD begin_sector_num = (begin_bytes % (BYTES_PER_SECTOR * fat16_ins->Bpb.BPB_SecPerClus)) / BYTES_PER_SECTOR;
  WORD end_sector_num = (end_bytes % (BYTES_PER_SECTOR * fat16_ins->Bpb.BPB_SecPerClus)) / BYTES_PER_SECTOR;
  WORD begin_bytes_num = begin_bytes % BYTES_PER_SECTOR;
  WORD end_bytes_num = end_bytes % BYTES_PER_SECTOR;

  DIR_ENTRY file_dir;
  off_t offset_dir;//similar to readdir
  find_root(fat16_ins, &file_dir, path, &offset_dir);
    
  WORD ClusterN, FatClusEntryVal, FirstSectorofCluster;

  ClusterN = file_dir.DIR_FstClusLO;
  first_sector_by_cluster(fat16_ins, ClusterN, &FatClusEntryVal, &FirstSectorofCluster, sector_buffer);

  for (int i = 0;i <= end_cluster_num;i++) {
    if (i >= begin_cluster_num) {
      WORD begin, end;
      //discuss the cons of 1st and the last 
      if (i == begin_cluster_num) begin = begin_sector_num;
      else begin = 0;
      if (i == end_cluster_num) end = end_sector_num;
      else end = fat16_ins->Bpb.BPB_SecPerClus - 1;
      //read the content of each cluster into buffer
      for (int j = begin;j <= end;j++) {
        sector_read(fat16_ins->fd, FirstSectorofCluster + j, sector_buffer);
        if (i == begin_cluster_num && j == begin_sector_num) {//1st
          memcpy(buffer + curr_buf_pos, (const void *)(sector_buffer + begin_bytes_num), BYTES_PER_SECTOR - begin_bytes_num);
          curr_buf_pos += BYTES_PER_SECTOR - begin_bytes_num;
        }
        else if (i == end_cluster_num && j == end_sector_num) {//last
          memcpy(buffer + curr_buf_pos, (const void *)sector_buffer, end_bytes_num + 1);
          curr_buf_pos += end_bytes_num + 1;
          return curr_buf_pos;
        }
        else {
          memcpy(buffer + curr_buf_pos, (const void *)sector_buffer, BYTES_PER_SECTOR);
          curr_buf_pos += BYTES_PER_SECTOR;
        }
      }
    }

    if (FatClusEntryVal == 0xFFFF) {//the last cluster
      return curr_buf_pos;
    } 
    else {
      ClusterN = FatClusEntryVal;
...
int fat16_mknod(const char *path, mode_t mode, dev_t devNum)
{
  ...
  WORD ClusterN, FatClusEntryVal, FirstSectorofCluster;
  const char* target_name = paths[pathDepth-1];
  ...
  for (i = 1; i <= fat16_ins->Bpb.BPB_RootEntCnt; i++)
    {
      memcpy(&Root, &sector_buffer[((i - 1) * BYTES_PER_DIR) % BYTES_PER_SECTOR], BYTES_PER_DIR);
      if (strncmp((const char*)Root.DIR_Name, target_name, 11) == 0)
      {
        findFlag = 0;
        break;
      }
  ...
  while (1)
  {
    for (size_t i = 0; i < fat16_ins->Bpb.BPB_SecPerClus; i++)
    {
      sector_read(fat16_ins->fd, FirstSectorofCluster + i, sector_buffer);
        ...
        {
          memcpy(&temp_DIR, sector_buffer+j, BYTES_PER_DIR);
          if (strncmp((const char*)temp_DIR.DIR_Name,target_name,sizeof(temp_DIR.DIR_Name))==0)
          {
            findFlag = 0;
            flag = 1;
            break;
          } 
          if (temp_DIR.DIR_Name[0] == 0x00 || temp_DIR.DIR_Name[0] == 0xe5)
          {
            findFlag = 1;
            flag = 1;
...
int dir_entry_create(FAT16 *fat16_ins, int sectorNum, int offset, char *Name, BYTE attr, WORD firstClusterNum, DWORD fileSize)
{
  ...
  ((DIR_ENTRY *)entry_info)->DIR_FileSize = fileSize;
  /*** END ***/
  ...
  fwrite(entry_info, sizeof(BYTE), 32, fd);
  fflush(fd);
  /*** END ***/
  free(entry_info);
  return 0;
...
int free_cluster(FAT16 *fat16_ins, int ClusterNum)
{
   ...
   **/
  /*** BEGIN ***/
  //last cluster/cluster no = 0
  WORD free_cluster = 0x0000;
  unsigned long cluster_offset1 = fat16_ins->Bpb.BPB_RsvdSecCnt * fat16_ins->Bpb.BPB_BytsPerSec + ClusterNum * fat16_ins->ClusterSize;
  unsigned long cluster_offset2 = cluster_offset1 + fat16_ins->Bpb.BPB_FATSz16 * fat16_ins->Bpb.BPB_BytsPerSec;
  fseek(fd, cluster_offset1, SEEK_SET);
  fwrite(&free_cluster, sizeof(WORD), 1, fd);
  fflush(fd);
  fseek(fd, cluster_offset2, SEEK_SET);
  fwrite(&free_cluster, sizeof(WORD), 1, fd);
  fflush(fd);
  /*** END ***/
  ...
  else /* Else if parent directory is sub-directory */
  {
    ...
    while (1)
    {
      for (size_t i = 0; i < fat16_ins->Bpb.BPB_SecPerClus; i++)
      {
        ...
        for (size_t j = 0; j < BYTES_PER_SECTOR; j += BYTES_PER_DIR)
        {
          memcpy(&tmp_dir, sector_buffer + j, BYTES_PER_DIR);
          if (tmp_dir.DIR_Name[0] == 0xe5||tmp_dir.DIR_Name[0] == 0x00)
            continue;
          
          if (strncmp(tmp_dir.DIR_Name, paths[pathDepth - 1], 11) == 0)
          {
            findFlag = 1;
            sectorNum = FirstSectorofCluster + i;
            offset = j;
