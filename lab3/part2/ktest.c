...
// 定义要输出的文件
#define OUTPUT_FILE "/home/lks/oslab/lab3.2/lab3-part2/expr_result.txt"
...
static my_page_referenced mpage_referenced = (my_page_referenced)0xffffffff8c6b0d10;
// sudo cat /proc/kallsyms | grep follow_page
static my_follow_page mfollow_page = (my_follow_page)0xffffffff8c694450;
...
static void scan_vma(void)
{
    ...
    if (mm)
    {
        ...
        int count=0;
        printk("first count=%d",count);
        struct vm_area_struct*vma_list=mm->mmap;
        while(vma_list!=NULL){
            
            my_task_info.vma_cnt=my_task_info.vma_cnt+1;
...
static void print_mm_active_info(void)
{
    ...
    if(mm){
        ...
        while(vma){
            ...
            while(vir_addr<=(vma->vm_end)){
                page=mfollow_page(vma,vir_addr,FOLL_GET);
                if(!IS_ERR_OR_NULL(page)){//useful
                    freq = mpage_referenced(page, 0, (struct mem_cgroup *)(page->memcg_data), &vm_flags);
                    if(freq){
                        record_one_data(page_to_pfn(page));//chage to phys
                    }
                }
                vir_addr+=PAGE_SIZE;
            }
...
static void traverse_page_table(struct task_struct *task)
{
    ...
    if (mm)
    {
        ...
        unsigned long vir_addr=0;
        while(vma){
            for(vir_addr=vma->vm_start;vir_addr<=(vma->vm_end);vir_addr+=PAGE_SIZE){
                //printk("start: %u   %u\n",virt_addr,virt2phys(task->mm, virt_addr));
                record_two_data(vir_addr,virt2phys(mm,vir_addr));
            }
...
static void print_seg_info(void)
{
    ...
    struct page *page=NULL;
    unsigned long start_code, end_code, start_data, end_data;//border
    unsigned long vir_addr=0;
    ...
    //initial finished
    vma=find_vma(mm,start_data);//data
    vir_addr=start_data;
    while((vma->vm_start)<=end_data){
        while((vir_addr<=end_data)&&(vir_addr<=vma->vm_end)){
            page=mfollow_page(vma,vir_addr,FOLL_GET);
            if(!IS_ERR_OR_NULL(page)){
                addr=kmap_atomic(page);
                memcpy(buf,addr,PAGE_SIZE);
                curr_buf_length = sizeof(buf);//set the len
    ...
    while((vma->vm_start<=end_code)){
        while((vir_addr<=end_code)&&(vir_addr<=vma->vm_end)){
            page=mfollow_page(vma,vir_addr,FOLL_GET);
            if(!IS_ERR_OR_NULL(page)){
                addr=kmap_atomic(page);
                memcpy(buf,addr,PAGE_SIZE);
