...
void *mm_malloc(size_t size)
{
    ...
    */
    ...
    //heap_size+=extend_size;//add the heap  
...
static void *extend_heap(size_t words)
{
    ...
    heap_size+=size;//add if heap is success to extend
...
static void *coalesce(void *bp)
{
    ...
    */
    ...
    {
        delete_from_free_list(NEXT_BLKP(bp));//pick up the next one
    ...
    else if (!prev_alloc && next_alloc) /*前块空闲，后块已分配*/
    {
        ...
        PUT(HDRP(PREV_BLKP(bp)),PACK(size,GET_PREV_ALLOC(HDRP(PREV_BLKP(bp))),0));
        PUT(FTRP(bp),PACK(size,GET_PREV_ALLOC(FTRP(PREV_BLKP(bp))),0));
    ...
    else /*前后都是空闲块*/
    {
        ...
        PUT(FTRP(NEXT_BLKP(bp)),PACK(size,GET_PREV_ALLOC(HDRP(PREV_BLKP(bp))),0));
        PUT(HDRP(PREV_BLKP(bp)),PACK(size,GET_PREV_ALLOC(HDRP(PREV_BLKP(bp))),0));
...
static void* find_fit_best(size_t asize) {
    ...
    */
    ...
    char*min_addr=NULL;
    while(current!=NULL){
        if((GET_SIZE(HDRP(current))>asize)&&((GET_SIZE(HDRP(current))<min))){
            min=GET_SIZE(HDRP(current));
            min_addr=(char*)current;
        }
        else{
            current=(char*)GET_SUCC(current);
        }
...
static void place(void *bp, size_t asize)
{
    ...
    */
    size_t bp_size,left;
    bp_size=GET_SIZE(HDRP(bp));//get the block size
    
    

    if(bp_size-asize<MIN_BLK_SIZE){
        PUT(HDRP(bp),PACK(bp_size,GET_PREV_ALLOC(HDRP(bp)),1));
        PUT(FTRP(bp),PACK(bp_size,GET_PREV_ALLOC(HDRP(bp)),1));
        ...
        //ulti
        user_malloc_size += bp_size;
    }
    else{
        delete_from_free_list(bp);
        char* new_bp = bp + asize;
        size_t new_size = bp_size - asize;
